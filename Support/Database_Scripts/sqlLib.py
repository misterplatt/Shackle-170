...
#originally written by Ryan Connors for Smart-Irrigation Backend UCSC Fall 2015 with a few fixes from Coy Humphrey
#pulled and woven into Shackle Game by Ryan Connors to capture metrics and bug data.
import ConfigParser
import MySQLdb
import os
import subprocess
...
def pushData ( rowDict , table=None, col_format=None) :
    global activeTable
    global colFormat
    global cursor

    if table is None: table = activeTable
    if col_format is None: col_format = colFormat
    ...
    addEntry = ("INSERT INTO " + table + " "
                "(" + jDelim.join(col_format) + ") "
                "VALUES (" + jDelim.join (map(lambda x: "%(" + x + ")s", col_format)) + ")")
    
    executeCommand( addEntry % rowDict)
